...
import com.menglingqiang.schedule.util.RedisClient;
...
public class UserMailController {
  ...
  public static final String passwordFailKey = "password_fail_";
  @Autowired
  ...
  //TODO 这里有点怪只能操作String不可以操作Integer的,看看解决办法
  @RequestMapping(value = "/verifyPasswordCount", method = {RequestMethod.GET})
  public boolean verifyPasswordCount(Long userId) {
    try {
      String key = passwordFailKey + userId;
  ...
  public @ResponseBody
  String getPicCode(HttpServletRequest request) {
    String picCode = (String) request.getSession().getAttribute("picCode");
    return picCode;
  }
  ...
  public @ResponseBody
  String isRegister(HttpServletRequest request)//前台会自动的将email注入到user中
  {
    String email = (String) request.getParameter("email");
    User user = new User();
    user.setEmail(email);
  ...
  public @ResponseBody
  String loginFlag(HttpServletRequest request, HttpServletResponse response) throws IOException {
  ...
  public @ResponseBody
  String registerFlag(HttpServletRequest request, HttpServletResponse response) throws IOException {
    //1,邮箱已经注册，0，邮箱没有注册
    String email = request.getParameter("email");
    User user = new User();
    user.setEmail(email);
